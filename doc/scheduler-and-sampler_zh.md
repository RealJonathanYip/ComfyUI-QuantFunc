...
### 1.1 确定性采样器（不受 `sampler_eta` / `sampler_s_noise` 影响）
...
|------|------|------|-------------------|------|
| `sampler_eta` | `0.0` | ≥0 | 随机 / 祖先 + SA-Solver（§1.2/§1.3） | 随机性（eta）噪声尺度。`0` = 确定性（对确定性采样器无影响） |
| `sampler_s_noise` | `1.0` | ≥0 | 同上（SDE / 祖先） | SDE 噪声倍数。`1.0` = ComfyUI 默认；`0` = 不注入噪声；`>1` = 更多噪声 |
